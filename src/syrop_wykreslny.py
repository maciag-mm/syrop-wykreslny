...
===============================================================================
 Samodzielny program dla Windows 10 / 11 (PyQt6 + GDAL) – wersja programu
 odpowiadająca skryptowi QGIS „Syrop Wykreślny”. Nie wymaga QGIS ani Pythona
 (dystrybuowany jako plik exe z instalatorem).

 Wejście:  linia / multilinia 3D (Z = wysokość) z pliku KML, DXF, DWG, SHP, GPKG
           w układzie EPSG 4326, 2176, 2177, 2178, 2179 lub 2180.
 Wyjście:  obraz (schowek / PNG), wydruk / PDF oraz KML, DXF, DWG, DGN, SHP, GPKG
           w EPSG 4326, 2176–2180 lub w układzie lokalnym (0,0 = początek wykresu).

 DWG: odczyt i zapis przez dołączony konwerter LibreDWG (GPL-3.0-or-later).
 Jeśli na komputerze jest zainstalowany ODA File Converter, program używa go
 automatycznie (zapis DWG w formacie AutoCAD 2018) – nie jest on jednak wymagany.

 Uruchomienie: SyropWykreslny.exe [plik_z_linią]

 Autor: Michał Maciąg

 Ten program jest wolnym oprogramowaniem: możesz go rozprowadzać dalej i/lub
 modyfikować na warunkach Powszechnej Licencji Publicznej GNU (GPL) w wersji 3
 lub (według Twojego wyboru) dowolnej późniejszej. Program rozpowszechniany jest
 BEZ JAKIEJKOLWIEK GWARANCJI. Szczegóły: plik LICENSE lub
...
def _setup_geo_data():
    """GDAL_DATA / PROJ_DATA dla wersji spakowanej (PyInstaller) i zwykłej."""
    roots = []
    ...
    try:
        ...
        if spec and spec.submodule_search_locations:
            roots.append(os.path.dirname(list(spec.submodule_search_locations)[0]))
    except Exception:
        pass
    for root in roots:
        for sub in (("osgeo", "data", "proj"), ("osgeo", "data", "share", "proj"), ("proj",),
                    ("share", "proj")):
            d = os.path.join(root, *sub)
            if os.path.isfile(os.path.join(d, "proj.db")):
                GEO_DATA["PROJ_LONG"] = d
                os.environ["PROJ_DATA"] = d
                os.environ["PROJ_LIB"] = d
                break
        for sub in (("osgeo", "data", "gdal"), ("osgeo", "data", "share", "gdal"), ("gdal",),
                    ("share", "gdal")):
            d = os.path.join(root, *sub)
            if os.path.isdir(d) and glob.glob(os.path.join(d, "*.csv")) + glob.glob(os.path.join(d, "*.json")):
                GEO_DATA["GDAL_LONG"] = d
                os.environ["GDAL_DATA"] = d
                break
        if "PROJ_DATA" in os.environ and "GDAL_DATA" in os.environ:
            break
    # Zmienne środowiskowe są czytane przez GDAL/PROJ w kodowaniu ANSI – ścieżka z polskimi
    # znakami (np. „Pobrane\Próba”) psuje odczyt proj.db. Dlatego do zmiennych trafia krótka
    # ścieżka 8.3 (bez polskich znaków), a pełna ścieżka UTF-8 jest też podawana przez API (niżej).
    for key in ("PROJ_DATA", "PROJ_LIB", "GDAL_DATA"):
        v = os.environ.get(key)
        if v and not v.isascii():
            os.environ[key] = _short_path(v)


def _short_path(p):
    """Krótka ścieżka Windows 8.3 (bez znaków spoza ASCII); poza Windows – bez zmian."""
    if os.name != "nt":
        return p
    try:
        import ctypes
...
GEO_DATA = {}
_setup_geo_data()
GEO_DATA.update({k: os.environ.get(k) for k in ("PROJ_DATA", "GDAL_DATA")})

from PyQt6.QtCore import (Qt, QByteArray, QCoreApplication, QLineF, QMarginsF, QPointF, QRectF, QSettings,
                          QSize, QSizeF, QUrl)
from PyQt6.QtGui import (QAction, QBrush, QColor, QDesktopServices, QFont, QFontMetricsF, QGuiApplication, QIcon,
                         QIntValidator, QImage, QPageLayout, QPageSize, QPainter, QPen, QPixmap, QPolygonF)
...
for _m in (gdal, ogr, osr):
    try:
        _m.UseExceptions()
    except Exception:
        pass

# Pełne ścieżki UTF-8 do danych PROJ i GDAL przez API (działa też w folderach z polskimi znakami)
try:
    if GEO_DATA.get("PROJ_LONG"):
        osr.SetPROJSearchPaths([GEO_DATA["PROJ_LONG"]])
except Exception:
    pass
try:
    if GEO_DATA.get("GDAL_LONG"):
        gdal.SetConfigOption("GDAL_DATA", GEO_DATA["GDAL_LONG"])
except Exception:
    pass
...
APP_ID = "MichalMaciag.SyropWykreslny"


def app_data_dir():
    """Katalog na dziennik błędów: %LOCALAPPDATA%\\SyropWykreslny."""
    base = os.environ.get("LOCALAPPDATA") or os.path.expanduser("~")
    d = os.path.join(base, "SyropWykreslny")
    try:
...
LOG_PATH = os.path.join(app_data_dir(), "syrop_wykreslny.log")
_log = logging.getLogger("syrop")
...
# ---------------------------------------------------------------------------
def find_oda():
    if os.environ.get("SYROP_NO_ODA") == "1":
        return None
...
def ask_oda(parent, optional=False):
    exe = find_oda()
    if exe:
        return exe
    if parent is None:
    ...
    if optional:
        r = QMessageBox.question(
            parent, SCRIPT_NAME,
            "Wbudowany konwerter DWG (LibreDWG) nie poradził sobie z tym plikiem.\n\n"
            "Jeśli masz darmowy program „ODA File Converter”, możesz go wskazać – zostanie użyty "
            "zamiast wbudowanego konwertera. Wskazać go teraz?")
...
def oda_convert(exe, src, out_type, out_ver="ACAD2018"):
    ...
    kw = {"timeout": 300, "capture_output": True, "stdin": subprocess.DEVNULL, "env": child_env()}
    if os.name == "nt":
...
# ---------------------------------------------------------------------------
#  LibreDWG (DWG ⇄ DXF) – dołączony do programu, nie wymaga instalacji
# ---------------------------------------------------------------------------
def _app_dirs():
    dirs = []
    env = os.environ.get("SYROP_LIBREDWG")
    if env:
        dirs.append(env)
    if getattr(sys, "frozen", False):
        dirs.append(os.path.join(getattr(sys, "_MEIPASS", ""), "libredwg"))
        dirs.append(os.path.join(os.path.dirname(sys.executable), "libredwg"))
    here = os.path.dirname(os.path.abspath(__file__))
    dirs.append(os.path.join(here, "libredwg"))
    return dirs
...
def find_libredwg():
    ...
    for d in _app_dirs():
        if d and os.path.isfile(os.path.join(d, exe)):
            return d
    w = shutil.which("dwg2dxf")
    return os.path.dirname(w) if w else None


def child_env(extra_path=None):
    """
    Środowisko dla programów zewnętrznych (ODA, LibreDWG) bez zmiennych Qt/PyInstaller
    tego programu – inaczej ODA File Converter (też Qt) próbuje ładować nasze wtyczki Qt.
    """
    env = {k: v for k, v in os.environ.items()
           if not (k.upper().startswith(("QT_", "QML", "_PYI", "PYINSTALLER", "SYROP_")))}
    if getattr(sys, "frozen", False):
        mei = getattr(sys, "_MEIPASS", "")
        env["PATH"] = os.pathsep.join(p for p in env.get("PATH", "").split(os.pathsep)
                                      if p and not (mei and p.startswith(mei)))
    if extra_path:
        env["PATH"] = extra_path + os.pathsep + env.get("PATH", "")
    return env


def _run_tool(args, cwd, extra_path=None):
    kw = {"timeout": 300, "capture_output": True, "stdin": subprocess.DEVNULL, "cwd": cwd,
...
def libredwg_convert(src, out_type):
    """
    DWG → DXF (dwg2dxf) lub DXF → DWG (dxf2dwg, AutoCAD 2000) programem LibreDWG.
    Pracuje na kopiach o prostych nazwach w katalogu tymczasowym (bez polskich znaków w ścieżce).
    """
    d = find_libredwg()
    if not d:
        raise RuntimeError("Brak dołączonego konwertera LibreDWG.")
    work = tempfile.mkdtemp(prefix="syrop_ldwg_")
    if out_type.upper() == "DXF":
        tool, inp, out = "dwg2dxf", "wejscie.dwg", "wynik.dxf"
    else:
        tool, inp, out = "dxf2dwg", "wejscie.dxf", "wynik.dwg"
    shutil.copyfile(src, os.path.join(work, inp))
    exe = os.path.join(d, tool + (".exe" if os.name == "nt" else ""))
    r = _run_tool([exe, "-y", "-o", out, inp], work, d)
    res = os.path.join(work, out)
    if not os.path.isfile(res) or os.path.getsize(res) < 100:
        msg = (r.stderr or r.stdout or b"").decode("utf-8", "replace").strip().splitlines()[-5:]
        raise RuntimeError(f"LibreDWG ({tool}) nie utworzył pliku wynikowego.\n" + "\n".join(msg))
    return res


def dxf_r2000_for_dwg(dx, path):
    """
    Zapis rysunku jako DXF AutoCAD 2000 (ezdxf) – wejście dla dxf2dwg.
    LibreDWG najlepiej konwertuje pełny DXF 2000 (uchwyty, słowniki), a nie uproszczony R12.
    Zwraca False, gdy ezdxf jest niedostępny (wtedy używany jest DXF R12).
    """
    try:
        import ezdxf
    ...
    try:
        ...
        pass
    for name, col in dx.layers.items():
        if name not in doc.layers:
            doc.layers.add(name, color=col)
    msp = doc.modelspace()
    amap = {(0, 0): TA.LEFT, (1, 0): TA.CENTER, (2, 0): TA.RIGHT,
            (0, 1): TA.BOTTOM_LEFT, (1, 1): TA.BOTTOM_CENTER, (2, 1): TA.BOTTOM_RIGHT,
            (0, 2): TA.MIDDLE_LEFT, (1, 2): TA.MIDDLE_CENTER, (2, 2): TA.MIDDLE_RIGHT,
            (0, 3): TA.TOP_LEFT, (1, 3): TA.TOP_CENTER, (2, 3): TA.TOP_RIGHT}
    for kind, layer, c in dx.items:
        at = {"layer": layer}
        if kind == "L":
            msp.add_line((c[0], c[1], c[2]), (c[3], c[4], c[5]), dxfattribs=at)
        elif kind == "O":
            msp.add_point((c[0], c[1], c[2]), dxfattribs=at)
        elif kind == "P":
            if any(abs(p[2]) > 0 for p in c):
                # LibreDWG zapisuje POLYLINE (3D) z podwojonymi wierzchołkami, co psuje plik
                # w AutoCAD-zie – linia 3D trafia do DWG jako ciąg odcinków 3D (LINE).
                for a, b in zip(c, c[1:]):
                    msp.add_line((a[0], a[1], a[2]), (b[0], b[1], b[2]), dxfattribs=at)
            else:
                msp.add_lwpolyline([(p[0], p[1]) for p in c], dxfattribs=at)
        elif kind == "T":
            x, y, z, h, txt, rot, ha, va, bx, by, _w = c
            at.update({"height": h, "rotation": float(rot or 0.0), "style": "Standard"})
            t = msp.add_text(txt, dxfattribs=at)
            al = amap.get((ha, va), TA.LEFT)
            if al == TA.LEFT:
                t.set_placement((bx, by, z))
            else:
                t.set_placement((x, y, z), align=al)
                t.dxf.insert = (bx, by, z)  # przybliżony punkt lewej linii bazowej
    doc.header["$EXTMIN"] = (dx.minx, dx.miny, dx.minz) if dx.minx != float("inf") else (0, 0, 0)
    doc.header["$EXTMAX"] = (dx.maxx, dx.maxy, dx.maxz) if dx.minx != float("inf") else (0, 0, 0)
    doc.saveas(path)
    return True
...
def dwg_engine_name():
    if find_oda():
        return "ODA File Converter (AutoCAD 2018)"
    if find_libredwg():
        return "LibreDWG (AutoCAD 2000)"
    return None


def write_dwg(dx, path, parent=None):
    """
    Zapis DWG: gdy na komputerze jest ODA File Converter – przez niego (format AutoCAD 2018),
    w przeciwnym razie dołączonym LibreDWG (format AutoCAD 2000, otwierany przez każdy AutoCAD).
    """
    tmpdir = tempfile.mkdtemp(prefix="syrop_dxf_")
    base = "przekroj"
    errors = []
    exe = find_oda()
    if exe:
        try:
            tmp = os.path.join(tmpdir, base + ".dxf")
            dx.save(tmp)
            dwg = oda_convert(exe, tmp, "DWG", "ACAD2018")
            _replace(dwg, path)
            return "ODA File Converter"
        except Exception as ex:
            errors.append(f"ODA: {ex}")
    if find_libredwg():
        try:
            tmp = os.path.join(tmpdir, base + "_2000.dxf")
            if not dxf_r2000_for_dwg(dx, tmp):
                tmp = os.path.join(tmpdir, base + ".dxf")
                dx.save(tmp)
            dwg = libredwg_convert(tmp, "DWG")
            _replace(dwg, path)
            return "LibreDWG"
        except Exception as ex:
            errors.append(f"LibreDWG: {ex}")
    if parent is not None:
        exe = ask_oda(parent, optional=True)
        if exe:
            tmp = os.path.join(tmpdir, base + ".dxf")
            dx.save(tmp)
            _replace(oda_convert(exe, tmp, "DWG", "ACAD2018"), path)
            return "ODA File Converter"
    raise RuntimeError("Nie udało się zapisać DWG.\n" + "\n".join(errors))
...
def read_lines(path, parent):
    ...
    if ext == ".dwg" and not feats:
        # GDAL czyta tylko DWG 2000 – nowsze wersje konwertujemy do DXF:
        # ODA File Converter (jeśli jest zainstalowany), a zawsze dołączony LibreDWG.
        errors = []
        engines = []
        if find_oda():
            engines.append(("ODA File Converter", lambda: oda_convert(find_oda(), path, "DXF")))
        if find_libredwg():
            engines.append(("LibreDWG", lambda: libredwg_convert(path, "DXF")))
        for name, conv in engines:
            try:
                dxf = conv()
                ds = open_ogr(dxf)
                if ds is None:
                    raise RuntimeError("nie można otworzyć przekonwertowanego DXF")
                with ogr_exceptions():
                    feats, epsg = scan_datasource(ds)
                ds = None
                if feats:
                    note = f"DWG odczytano przez {name}."
                    break
                errors.append(f"{name}: brak linii po konwersji")
            except Exception as ex:
                errors.append(f"{name}: {ex}")
        if not feats:
            exe = ask_oda(parent, optional=True) if not find_oda() else None
            if exe:
        ...
        if not feats and errors:
            raise RuntimeError("Nie udało się odczytać pliku DWG.\n" + "\n".join(errors))
    if ext in (".kml", ".kmz"):
...
#  Model przekroju
# ---------------------------------------------------------------------------
def measure_line(x1, y1, x2, y2, geographic):
    """
    Odległość między punktami – jak QgsDistanceArea w skrypcie QGIS:
    układ geograficzny → linia geodezyjna na elipsoidzie WGS 84 (GeographicLib),
    układ płaski → odległość kartezjańska w jednostkach układu.
    """
    if not geographic:
        return math.hypot(x2 - x1, y2 - y1)
    if Geodesic is not None:
        return Geodesic.WGS84.Inverse(y1, x1, y2, x2, Geodesic.DISTANCE)["s12"]
    # zapas: wzór Vincenty'ego (odwrotny) na elipsoidzie WGS 84
    ...
    for _ in range(200):
        ...
        lp, lam = lam, L + (1 - C) * f * sa * (sig + C * ss * (c2m + C * cs * (-1 + 2 * c2m * c2m)))
        if abs(lam - lp) < 1e-12:
            break
    u2 = c2a * (a * a - b * b) / (b * b)
    A = 1 + u2 / 16384 * (4096 + u2 * (-768 + u2 * (320 - 175 * u2)))
    B = u2 / 1024 * (256 + u2 * (-128 + u2 * (74 - 47 * u2)))
    ds = B * ss * (c2m + B / 4 * (cs * (-1 + 2 * c2m * c2m) - B / 6 * c2m * (-3 + 4 * ss * ss) *
                                  (-3 + 4 * c2m * c2m)))
    return b * A * (sig - ds)


def make_transform(src_epsg, dst_epsg):
    """Transformacja współrzędnych (PROJ przez GDAL/OSR); None gdy układy są takie same."""
    if int(src_epsg) == int(dst_epsg):
        return None
    return osr.CoordinateTransformation(_srs(src_epsg), _srs(dst_epsg))
...
class Profile:
    ...
    def __init__(self, name, epsg, parts_xyz):
        ...
        self.epsg = epsg
        self.crs = _srs(epsg)
        geographic = bool(self.crs.IsGeographic())
        self.parts = []
        d, prev = 0.0, None
        for part in parts_xyz:
            ...
            for x, y, z in part:
                if z is None or (isinstance(z, float) and math.isnan(z)):
                    continue
                if prev is not None:
                    d += measure_line(prev[0], prev[1], x, y, geographic)
    ...
    def paintEvent(self, e):
        ...
        try:
            ...
            try:
                ...
                p.drawText(QPointF(20, 30), f"Błąd rysowania – szczegóły w pliku: {LOG_PATH}")
            except Exception:
                pass
        finally:
            p.end()
...
class InputDialog(QDialog):
    def __init__(self, parent=None):
        ...
        head.addWidget(t, 1)
        lic = QLabel(f"<span style='color:#777'>wersja {SCRIPT_VERSION}<br>© Michał Maciąg<br>"
                     "<a href='https://www.gnu.org/licenses/gpl-3.0.html'>GPL-3.0-or-later</a>, "
                     "bez gwarancji</span>")
        lic.setOpenExternalLinks(True)
        lic.setAlignment(Qt.AlignmentFlag.AlignRight | Qt.AlignmentFlag.AlignTop)
        lic.setToolTip("Wolne oprogramowanie na licencji GNU GPL v3 lub późniejszej. Licencje użytych "
                       "komponentów: folder „licencje” obok programu.")
        head.addWidget(lic)

        self.ed_path = QLineEdit()
        self.ed_path.setPlaceholderText("KML, DXF, DWG, SHP lub GPKG z linią / multilinią 3D (można przeciągnąć plik)")
    ...
    # --- przeciągnij i upuść plik na okno
...
class ExportDialog(QDialog):
    def __init__(self, profile, parent=None, sheet_on=False):
        ...
        row.addWidget(self.ed_path, 1)
        row.addWidget(bt)
        self.chk_add = QCheckBox("Po zapisie pokaż plik w Eksploratorze Windows")
    ...
    def refresh(self):
        ...
        if fmt == "DWG":
            eng = dwg_engine_name()
            d += (f" DWG zapisuje: {eng}." if eng else " Brak konwertera DWG.")
    ...
    with ogr_exceptions():
        if fmt == "SHP":
            ...
            for suf, fn in jobs:
                ...
                if suf == "opisy":
                    try:  # QGIS wczytuje automatycznie styl <nazwa>.qml leżący obok pliku SHP
                        with open(os.path.splitext(pth)[0] + ".qml", "w", encoding="utf-8") as fh:
                            fh.write(text_layer_qml())
                    except Exception:
                        report_error("Styl etykiet SHP")
            return outs
        if fmt == "GPKG":
            ...
            ds = None
            if any(o[2] == "opisy" for o in outs):
                write_gpkg_style(path, "przekroj_opisy", text_layer_qml())
            return outs
        if fmt == "KML":
...
# ---------------------------------------------------------------------------
#  Styl QGIS dla warstwy opisów (etykiety) – QGIS wczyta go sam przy otwarciu:
#  SHP → plik <nazwa>.qml obok, GPKG → tabela layer_styles (styl domyślny).
# ---------------------------------------------------------------------------
def _dd(name, expr=None, field=None):
    from xml.sax.saxutils import quoteattr
    if field is not None:
        return (f'<Option name="{name}" type="Map"><Option name="active" type="bool" value="true"/>'
                f'<Option name="field" type="QString" value={quoteattr(field)}/>'
                f'<Option name="type" type="int" value="2"/></Option>')
    return (f'<Option name="{name}" type="Map"><Option name="active" type="bool" value="true"/>'
            f'<Option name="expression" type="QString" value={quoteattr(expr)}/>'
            f'<Option name="type" type="int" value="3"/></Option>')


def text_layer_qml():
    """QML: etykiety z pola „nazwa”, wysokość/obrót/wyrównanie z atrybutów, niewidoczne punkty."""
    props = "".join([_dd("Hali", field="hali"),
                     _dd("LabelRotation", expr='-"obrot"'),
                     _dd("PositionX", expr="x($geometry)"),
                     _dd("PositionY", expr="y($geometry)"),
                     _dd("Size", expr='"wys_txt" * 1.4'),
                     _dd("Vali", field="vali")])
    return f"""<!DOCTYPE qgis PUBLIC 'http://mrcc.com/qgis.dtd' 'SYSTEM'>
<qgis version="3.40.0" styleCategories="Symbology|Labeling" labelsEnabled="1">
 <renderer-v2 type="singleSymbol" forceraster="0" symbollevels="0" enableorderby="0" referencescale="-1">
  <symbols>
   <symbol type="marker" name="0" alpha="0" clip_to_extent="1" force_rhr="0" frame_rate="10" is_animated="0">
    <data_defined_properties><Option type="Map"><Option name="name" type="QString" value=""/>
     <Option name="properties"/><Option name="type" type="QString" value="collection"/></Option></data_defined_properties>
 ...
 <labeling type="simple">
  <settings calloutType="simple">
   <text-style fieldName="nazwa" isExpression="0" fontFamily="Arial" fontSize="1" fontSizeUnit="MapUnit"
               fontWeight="50" fontItalic="0" textColor="0,0,0,255" textOpacity="1" multilineHeight="1"
               allowHtml="0" namedStyle="Regular">
    <families/>
    <text-buffer bufferDraw="0" bufferSize="1" bufferSizeUnits="MM" bufferColor="255,255,255,255"/>
    <text-mask maskEnabled="0"/>
    <background shapeDraw="0"/>
    <shadow shadowDraw="0"/>
    <dd_properties><Option type="Map"><Option name="name" type="QString" value=""/><Option name="properties"/>
     <Option name="type" type="QString" value="collection"/></Option></dd_properties>
    <substitutions/>
   </text-style>
   <text-format wrapChar="" multilineAlign="0" autoWrapLength="0" useMaxLineLengthForAutoWrap="1"
                addDirectionSymbol="0" formatNumbers="0" decimals="3" plussign="0" reverseDirectionSymbol="0"
                placeDirectionSymbol="0" leftDirectionSymbol="&lt;" rightDirectionSymbol="&gt;"/>
   <placement placement="1" offsetType="0" dist="0" quadOffset="4" xOffset="0" yOffset="0"
              rotationAngle="0" priority="5" fitInPolygonOnly="0" centroidWhole="0" centroidInside="0"
              repeatDistance="0" maxCurvedCharAngleIn="25" maxCurvedCharAngleOut="-25" predefinedPositionOrder="TR,TL,BR,BL,R,L,TSR,BSR"
              overlapHandling="AllowOverlapIfRequired" allowDegraded="1"/>
   <rendering obstacle="0" displayAll="1" upsidedownLabels="2" labelPerPart="0" mergeLines="0"
              scaleVisibility="0" fontLimitPixelSize="0" fontMinPixelSize="1" fontMaxPixelSize="10000"
              limitNumLabels="0" maxNumLabels="2000" drawLabels="1" unplacedVisibility="1"
              obstacleType="1" obstacleFactor="1" zIndex="0" minFeatureSize="0"/>
   <dd_properties>
    <Option type="Map">
     <Option name="name" type="QString" value=""/>
     <Option name="properties" type="Map">{props}</Option>
...
def write_gpkg_style(path, table, qml):
    """Zapisuje styl QGIS (domyślny) w tabeli layer_styles pliku GeoPackage."""
    try:
        with ogr_exceptions():
            ds = ogr.Open(path, 1)
            lyr = ds.GetLayerByName("layer_styles")
            if lyr is None:
                lyr = ds.CreateLayer("layer_styles", None, ogr.wkbNone, options=["FID=id"])
                for fname, ftype, width in (("f_table_catalog", ogr.OFTString, 256),
                                            ("f_table_schema", ogr.OFTString, 256),
                                            ("f_table_name", ogr.OFTString, 256),
                                            ("f_geometry_column", ogr.OFTString, 256),
                                            ("styleName", ogr.OFTString, 30),
                                            ("styleQML", ogr.OFTString, 0),
                                            ("styleSLD", ogr.OFTString, 0),
                                            ("useAsDefault", ogr.OFTInteger, 0),
                                            ("description", ogr.OFTString, 0),
                                            ("owner", ogr.OFTString, 30),
                                            ("ui", ogr.OFTString, 30),
                                            ("update_time", ogr.OFTDateTime, 0)):
                    fd = ogr.FieldDefn(fname, ftype)
                    if width:
                        fd.SetWidth(width)
                    if fname == "useAsDefault":
                        fd.SetSubType(ogr.OFSTBoolean)
                    lyr.CreateField(fd)
            gcol = ""
            tl = ds.GetLayerByName(table)
            if tl is not None:
                gcol = tl.GetGeometryColumn() or "geom"
            f = ogr.Feature(lyr.GetLayerDefn())
            f.SetField("f_table_catalog", "")
            f.SetField("f_table_schema", "")
            f.SetField("f_table_name", table)
            f.SetField("f_geometry_column", gcol)
            f.SetField("styleName", table)
            f.SetField("styleQML", qml)
            f.SetField("styleSLD", "")
            f.SetField("useAsDefault", 1)
            f.SetField("description", f"{SCRIPT_NAME} – etykiety opisów")
            f.SetField("owner", "")
            f.SetField("ui", "")
            import datetime
            n = datetime.datetime.now()
            f.SetField("update_time", n.year, n.month, n.day, n.hour, n.minute, n.second, 0)
            lyr.CreateFeature(f)
            ds = None
    except Exception:
        report_error("Styl etykiet w GeoPackage")
...
def _open_windows():
    """Rejestr otwartych okien – chroni obiekty Pythona przed usunięciem przez odśmiecacz pamięci."""
...
class ProfileWindow(QDialog):
    def __init__(self, profile, parent=None):
        ...
        b_new = QPushButton("Nowy przekrój…")
        b_new.setToolTip("Otwiera okno wyboru pliku i rysuje kolejny przekrój w nowym oknie")
        b_new.clicked.connect(self.new_profile)
        ...
        btns = QGridLayout()
        for n, b in enumerate((b_copy, b_head, b_png, b_sheet, b_exp, b_print, b_new, b_close)):
    ...
    def export(self):
        ...
        if dlg.chk_add.isChecked():
            reveal_in_explorer(sorted({o[0] for o in outs})[0])
    ...
    def do_export(self, fmt, epsg, path, el=None):
        ...
        if epsg == LOCAL:
            ...
            ct = make_transform(pr.epsg, epsg)
            parts = []
            for p in pr.parts:
                q = []
                for x, y, z, _d in p:
        ...
        if fmt == "DXF":
            dx.save(path)
            return [(path, None, "cad")]
        write_dwg(dx, path, self)
        return [(path, None, "cad")]

    # --- nowy przekrój (odpowiednik ikony na pasku narzędzi QGIS)
    def new_profile(self):
        start_profile(self)


def reveal_in_explorer(path):
    """Otwiera Eksplorator Windows z zaznaczonym plikiem (inne systemy: folder)."""
    try:
        ...
        report_error("Eksplorator")


# ---------------------------------------------------------------------------
#  Uruchomienie
# ---------------------------------------------------------------------------
def start_profile(parent=None, path=None):
    """Okno danych wejściowych → okno przekroju (jak kliknięcie ikony w QGIS)."""
    dlg = InputDialog(None)
    if path:
        dlg.ed_path.setText(path)
        dlg.load_file()
    ok = dlg.exec()
    profile = dlg.profile
    dlg.deleteLater()
    if not ok or profile is None:
        return None
    win = ProfileWindow(profile, None)
    win.show()
    win.raise_()
    win.activateWindow()
    return win
...
def _excepthook(etype, value, tb):
    """Nieobsłużony wyjątek: zapis do dziennika i komunikat zamiast zamknięcia programu."""
    txt = "".join(traceback.format_exception(etype, value, tb))
    try:
        _log.error("Nieobsłużony wyjątek:\n" + txt)
    except Exception:
        pass
    try:
        QGuiApplication.restoreOverrideCursor()
        box = QMessageBox(QMessageBox.Icon.Critical, SCRIPT_NAME,
...
def selftest(outdir):
    """
    Test działania bez okien (używany przy budowaniu): odczyt KML/GPKG/SHP/DXF,
    przekrój, obraz, PDF i eksport do wszystkich formatów (bez DWG).
    Wynik: <outdir>/selftest.txt, kod wyjścia 0 = wszystko działa.
    """
    ...
    rep.append(f"{SCRIPT_NAME} {SCRIPT_VERSION}  GDAL {gdal.__version__}  Python {sys.version.split()[0]}")
    rep.append(f"GDAL_DATA={os.environ.get('GDAL_DATA')}  PROJ_DATA={os.environ.get('PROJ_DATA')}")
    rep.append("Sterowniki: " + ", ".join(d for d in ("KML", "LIBKML", "DXF", "CAD", "DGN", "GPKG", "ESRI Shapefile")
                                         if ogr.GetDriverByName(d) is not None))
    # linia testowa w PL-1992 (dwie części) i ta sama linia w WGS 84
    parts_2180 = [[(560000.0, 250000.0, 210.5), (560120.0, 250050.0, 214.2), (560300.0, 250020.0, 208.9)],
                  [(560300.0, 250020.0, 208.9), (560420.0, 250150.0, 221.7), (560600.0, 250160.0, 219.3)]]
    inputs = {}
    try:
        ct = make_transform(2180, 4326)
        parts_4326 = [[ct.TransformPoint(x, y)[:2] + (z,) for x, y, z in p] for p in parts_2180]
        ok("transformacja 2180 → 4326: " + ", ".join(f"{v:.6f}" for v in parts_4326[0][0][:2]))
    except Exception:
        bad("transformacja układów (PROJ)")
        parts_4326 = None
    try:
        pth = os.path.join(outdir, "wej.gpkg")
        drv = ogr.GetDriverByName("GPKG")
        if os.path.exists(pth):
            drv.DeleteDataSource(pth)
        ds = drv.CreateDataSource(pth)
        _write_line_layer(ds, "linia", _srs(2180), parts_2180, True, {"nazwa": "test"})
        ds = None
        inputs["GPKG"] = (pth, 2180)
        pth = os.path.join(outdir, "wej.shp")
        drv = ogr.GetDriverByName("ESRI Shapefile")
        if os.path.exists(pth):
            drv.DeleteDataSource(pth)
        ds = drv.CreateDataSource(pth)
        _write_line_layer(ds, "wej", _srs(2180), parts_2180, True, {"nazwa": "test"})
        ds = None
        inputs["SHP"] = (pth, 2180)
        pth = os.path.join(outdir, "wej.dxf")
        dx = DxfR12()
        for p in parts_2180:
            dx.polyline(p, "0", three_d=True)
        dx.save(pth)
        inputs["DXF"] = (pth, None)
        if parts_4326:
            pth = os.path.join(outdir, "wej.kml")
            if os.path.exists(pth):
                os.remove(pth)
            ds = ogr.GetDriverByName("KML").CreateDataSource(pth)
            _write_line_layer(ds, "linia", _srs(4326), parts_4326, True, {"nazwa": "test"})
            ds = None
            inputs["KML"] = (pth, 4326)
        ok("pliki wejściowe utworzone")
    except Exception:
        bad("tworzenie plików wejściowych")

    profiles = {}
    for k, (pth, exp_epsg) in inputs.items():
        try:
            feats, epsg, _note = read_lines(pth, None)
            assert feats, "brak linii"
            parts = [pt for f in feats for pt in f.parts]
            use = epsg or 2180
            pr = Profile(k, use, parts)
            profiles[k] = pr
            ok(f"odczyt {k}: linii {len(feats)}, EPSG {epsg}, długość {pr.length:.3f} m, "
               f"H {pr.hmin:.2f}–{pr.hmax:.2f}, wierzchołków {pr.nverts}")
            if exp_epsg is not None and epsg != exp_epsg:
                raise RuntimeError(f"wykryto EPSG {epsg}, oczekiwano {exp_epsg}")
        except Exception:
            bad(f"odczyt {k}")
    if "GPKG" in profiles and "KML" in profiles:
        a, b = profiles["GPKG"].length, profiles["KML"].length
        rep.append(f"      długość PL-1992 {a:.3f} m, WGS 84 (geodezyjna) {b:.3f} m, różnica {100 * (b - a) / a:.3f} %")
        if abs(b - a) / a > 0.002:
            fails.append("długość geodezyjna odbiega od płaskiej o więcej niż 0,2 %")

    pr = profiles.get("GPKG") or next(iter(profiles.values()), None)
    if pr is None:
        fails.append("brak przekroju do dalszych testów")
    else:
        win = None
        try:
            win = ProfileWindow(pr)
            win.canvas.sheet.enabled = True
            win.canvas.update_extent()
            img = win.canvas.render_image()
            assert img.save(os.path.join(outdir, "obraz.png"))
            ok(f"obraz przekroju {img.width()} × {img.height()} px")
        except Exception:
            bad("okno przekroju / obraz")
        if win is not None:
            try:
                if QPrinter is None:
                    raise RuntimeError("brak QtPrintSupport")
                printer = QPrinter(QPrinter.PrinterMode.HighResolution)
                win.setup_page(printer)
                printer.setOutputFormat(QPrinter.OutputFormat.PdfFormat)
                pdf = os.path.join(outdir, "wydruk.pdf")
                printer.setOutputFileName(pdf)
                win.canvas.print_sheet(printer, "paper")
                assert os.path.getsize(pdf) > 1000
                ok(f"PDF {os.path.getsize(pdf)} B")
            except Exception:
                bad("wydruk PDF")
            el_all = {k for k, _l, _d in EXPORT_ELEMENTS}
            for fmt in ("DXF", "DGN", "GPKG", "SHP", "KML"):
                for epsg in (LOCAL, 2180, 2178, 4326):
                    if fmt == "KML" and epsg != 4326:
                        continue
                    name = f"wyj_{fmt}_{epsg or 'lokalny'}.{fmt.lower()}"
                    try:
                        outs = win.do_export(fmt, epsg, os.path.join(outdir, name), el_all)
                        cnt = []
                        for o in outs:
                            ds = ogr.Open(o[0])
                            lyrs = [ds.GetLayerByName(o[1])] if o[1] else [ds.GetLayer(i) for i in range(ds.GetLayerCount())]
                            cnt.append(sum(lz.GetFeatureCount() for lz in lyrs if lz is not None))
                            ds = None
                        assert sum(cnt) > 0, "pusty wynik"
                        ok(f"eksport {fmt:5s} {('EPSG:%d' % epsg) if epsg else 'lokalny':10s} → obiektów {sum(cnt)}")
                    except Exception:
                        bad(f"eksport {fmt} {epsg}")
            # --- DWG bez ODA (LibreDWG) + kontrola zgodności ODA-em, jeśli jest na komputerze
            rep.append(f"      LibreDWG: {find_libredwg()}   ODA: {find_oda()}")

            def count_dxf(pth):
                ds = ogr.Open(pth)
                n = sum(ds.GetLayer(i).GetFeatureCount() for i in range(ds.GetLayerCount()))
                ds = None
                return n

            os.environ["SYROP_NO_ODA"] = "1"
            dwgs = {}
            for epsg in (LOCAL, 2180):
                nm = f"wyj_DWG_{epsg or 'lokalny'}"
                try:
                    pth = os.path.join(outdir, nm + ".dwg")
                    win.do_export("DWG", epsg, pth, el_all)
                    dwgs[epsg] = pth
                    n_dxf = count_dxf(os.path.join(outdir, f"wyj_DXF_{epsg or 'lokalny'}.dxf"))
                    n_back = count_dxf(libredwg_convert(pth, "DXF"))
                    feats, _e, note = read_lines(pth, None)
                    assert feats, "brak linii po odczycie DWG"
                    ok(f"eksport DWG  {('EPSG:%d' % epsg) if epsg else 'lokalny':10s} (LibreDWG, {os.path.getsize(pth)} B)"
                       f" → z powrotem obiektów {n_back} (DXF: {n_dxf}), linii {len(feats)} {note}")
                    if n_back < n_dxf:
                        fails.append(f"DWG {epsg}: mniej obiektów niż w DXF")
                except Exception:
                    bad(f"eksport DWG {epsg} przez LibreDWG")
            os.environ.pop("SYROP_NO_ODA", None)
            oda = find_oda()
            if oda:
                for epsg, pth in dwgs.items():
                    try:
                        n = count_dxf(oda_convert(oda, pth, "DXF"))
                        n_dxf = count_dxf(os.path.join(outdir, f"wyj_DXF_{epsg or 'lokalny'}.dxf"))
                        ok(f"kontrola ODA: DWG z LibreDWG ({epsg or 'lokalny'}) otwiera się, obiektów {n} (DXF: {n_dxf})")
                        if n < n_dxf:
                            fails.append(f"ODA widzi w DWG z LibreDWG ({epsg}) mniej obiektów niż w DXF")
                    except Exception:
                        bad(f"kontrola ODA pliku DWG {epsg}")
                try:
                    d18 = oda_convert(oda, inputs["DXF"][0], "DWG", "ACAD2018")
                    os.environ["SYROP_NO_ODA"] = "1"
                    feats, _e, note = read_lines(d18, None)
                    pr3 = Profile("dwg2018", 2180, [pt for f in feats for pt in f.parts])
                    ok(f"odczyt DWG 2018 bez ODA: długość {pr3.length:.3f} m, H {pr3.hmin:.2f}–{pr3.hmax:.2f} {note}")
                except Exception:
                    bad("odczyt DWG 2018 bez ODA")
                finally:
                    os.environ.pop("SYROP_NO_ODA", None)
            try:
                win.close()
                ...
                pass
    rep.append("WYNIK: " + ("WSZYSTKO DZIAŁA" if not fails else f"BŁĘDY: {len(fails)}"))
    with open(os.path.join(outdir, "selftest.txt"), "w", encoding="utf-8") as fh:
        fh.write("\n".join(rep) + "\n")
...
def main(argv=None):
    ...
    if os.name == "nt":
        try:  # własna ikona na pasku zadań (a nie ikona Pythona)
            import ctypes
    ...
    if "--selftest" in argv:
        ...
        test_dir = argv[i + 1] if i + 1 < len(argv) else os.path.join(tempfile.gettempdir(), "syrop_selftest")
        if os.name != "nt":  # w Windows natywna platforma Qt (prawdziwe czcionki), okna nie są pokazywane
            os.environ["QT_QPA_PLATFORM"] = "offscreen"
    QCoreApplication.setOrganizationName("SyropWykreslny" if test_dir is None else "SyropWykreslny-test")
    QCoreApplication.setApplicationName("SyropWykreslny")
    app = QApplication.instance() or QApplication(argv)
    if test_dir is not None:
        return selftest(test_dir)
    app.setApplicationDisplayName(SCRIPT_NAME)
    app.setApplicationVersion(SCRIPT_VERSION)
    app.setWindowIcon(syrop_icon())
    sys.excepthook = _excepthook
    path = next((a for a in argv[1:] if os.path.isfile(a)), None)
    win = start_profile(None, path)
    if win is None:
        return 0
    return app.exec()
